...
def load_image(image_path, img_size):
    ...
    transform = transforms.Compose([
        transforms.Resize((img_size[1], img_size[0])),  # Resize to model's expected input size
        transforms.ToTensor(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default=r'D:\lian\Documents\backup\thesis\yolov8n_sppfcspc_mpdiou_2.5prune\yolov8n-sppfcspc_MPDIoU_2.5-lamp-exp3-finetune\weights\best.pt', help='trained weights path')
    # parser.add_argument('--weights', type=str, default=r'C:\Users\lian\PycharmProjects\ultralytics-distil\runs\distill\yolov8n_v3_sp_prune_s2\weights\best.pt', help='trained weights path')
    ...
    parser.add_argument('--image_folder', type=str, default=r'D:\lian\datasets\Data_v2\test\images', help='folder path for test images')
    ...
    image_paths = list(Path(opt.image_folder).glob('*.jpg'))  # Assuming images are .jpg, modify if necessary
    assert len(image_paths) > 0, f"No images found in {opt.image_folder}"

    # Choose the first image for testing
    example_inputs = load_image(image_paths[0], opt.imgs).to(device)
